...
test("landing page exposes the current OfficeKonnect product without runtime errors", async ({ page }) => {
  const failures = captureRuntimeErrors(page);
  await page.goto("/");

  await expect(page).toHaveTitle("OfficeKonnect");
  await expect(page.getByRole("heading", { name: /One workspace for every office task/i })).toBeVisible();
...
test("login route renders real authentication controls without runtime errors", async ({ page }) => {
  const failures = captureRuntimeErrors(page);
