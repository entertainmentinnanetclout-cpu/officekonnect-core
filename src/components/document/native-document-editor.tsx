...
function serializeEditor(root: HTMLDivElement, page: NativeDocumentContent["page"]): NativeDocumentContent {
  ...
  for (const node of Array.from(root.childNodes)) {
    ...
    if (tag === "TABLE") {
      const rows = Array.from(node.querySelectorAll(":scope > tbody > tr, :scope > tr")).map((row) =>
...
export function NativeDocumentEditor({ document, onDocumentUpdated }: NativeDocumentEditorProps) {
  ...
  const latestContentRef = useRef<NativeDocumentContent>(normalizeNativeDocumentContent(document.content));
  ...
  }, [document.id, document.content, document.editor_version, document.title, applyContentToEditor]);
    ...
    <div className="flex min-h-0 flex-1 flex-col bg-slate-100 dark:bg-slate-950">
      <div className="border-b bg-white px-3 py-2 dark:bg-slate-900">
        ...
        <div className="mt-2 flex flex-wrap items-center gap-1 border-t pt-2">
          <ToolbarButton label="Undo" onPress={() => exec("undo")}><Undo2 className="h-4 w-4" /></ToolbarButton>
          <ToolbarButton label="Redo" onPress={() => exec("redo")}><Redo2 className="h-4 w-4" /></ToolbarButton>
          <span className="mx-1 h-5 w-px bg-border" />
          <ToolbarButton label="Heading 1" onPress={() => exec("formatBlock", "H1")}><Heading1 className="h-4 w-4" /></ToolbarButton>
          <ToolbarButton label="Heading 2" onPress={() => exec("formatBlock", "H2")}><Heading2 className="h-4 w-4" /></ToolbarButton>
          <ToolbarButton label="Heading 3" onPress={() => exec("formatBlock", "H3")}><Heading3 className="h-4 w-4" /></ToolbarButton>
          <ToolbarButton label="Paragraph" onPress={() => exec("formatBlock", "P")}><PanelTop className="h-4 w-4" /></ToolbarButton>
          <span className="mx-1 h-5 w-px bg-border" />
          <ToolbarButton label="Bold" onPress={() => exec("bold")}><Bold className="h-4 w-4" /></ToolbarButton>
          <ToolbarButton label="Italic" onPress={() => exec("italic")}><Italic className="h-4 w-4" /></ToolbarButton>
          <ToolbarButton label="Underline" onPress={() => exec("underline")}><Underline className="h-4 w-4" /></ToolbarButton>
          <ToolbarButton label="Strikethrough" onPress={() => exec("strikeThrough")}><Strikethrough className="h-4 w-4" /></ToolbarButton>
          <label className="relative flex h-8 w-8 cursor-pointer items-center justify-center rounded-md hover:bg-accent" title="Text color">
            ...
            <input type="color" className="absolute inset-0 cursor-pointer opacity-0" onChange={(event) => exec("foreColor", event.target.value)} />
          </label>
          <label className="relative flex h-8 w-8 cursor-pointer items-center justify-center rounded-md hover:bg-accent" title="Highlight color">
            <span className="rounded bg-yellow-200 px-1 text-xs text-slate-900">A</span>
            <input type="color" className="absolute inset-0 cursor-pointer opacity-0" onChange={(event) => exec("hiliteColor", event.target.value)} />
          </label>
          <ToolbarButton label="Link" onPress={() => setLinkOpen(true)}><Link2 className="h-4 w-4" /></ToolbarButton>
          <span className="mx-1 h-5 w-px bg-border" />
          <ToolbarButton label="Align left" onPress={() => exec("justifyLeft")}><AlignLeft className="h-4 w-4" /></ToolbarButton>
          <ToolbarButton label="Align center" onPress={() => exec("justifyCenter")}><AlignCenter className="h-4 w-4" /></ToolbarButton>
          <ToolbarButton label="Align right" onPress={() => exec("justifyRight")}><AlignRight className="h-4 w-4" /></ToolbarButton>
          <ToolbarButton label="Justify" onPress={() => exec("justifyFull")}><AlignJustify className="h-4 w-4" /></ToolbarButton>
          <ToolbarButton label="Decrease indent" onPress={() => exec("outdent")}><IndentDecrease className="h-4 w-4" /></ToolbarButton>
          <ToolbarButton label="Increase indent" onPress={() => exec("indent")}><IndentIncrease className="h-4 w-4" /></ToolbarButton>
          <span className="mx-1 h-5 w-px bg-border" />
          <ToolbarButton label="Bulleted list" onPress={() => exec("insertUnorderedList")}><List className="h-4 w-4" /></ToolbarButton>
          <ToolbarButton label="Numbered list" onPress={() => exec("insertOrderedList")}><ListOrdered className="h-4 w-4" /></ToolbarButton>
          <ToolbarButton label="Quote" onPress={() => exec("formatBlock", "BLOCKQUOTE")}><Quote className="h-4 w-4" /></ToolbarButton>
          <ToolbarButton label="Horizontal rule" onPress={() => exec("insertHorizontalRule")}><Minus className="h-4 w-4" /></ToolbarButton>
          <ToolbarButton
            label="Insert table"
            onPress={() =>
              insertHtml(
          ...
          ><Table2 className="h-4 w-4" /></ToolbarButton>
          <Button variant="ghost" size="sm" className="h-8 px-2 text-xs" onClick={() => insertHtml('<div data-page-break="true" contenteditable="false"><span>Page break</span></div><p><br></p>')}>
            Page break
          </Button>
          <ToolbarButton label="Find and replace" onPress={() => setFindOpen((open) => !open)}><Search className="h-4 w-4" /></ToolbarButton>
        </div>

        {findOpen && (
          <div className="mt-2 flex flex-wrap items-center gap-2 rounded-lg border bg-muted/30 p-2">
            <Input value={findText} onChange={(event) => setFindText(event.target.value)} placeholder="Find" className="h-8 w-48" />
            <Input value={replaceText} onChange={(event) => setReplaceText(event.target.value)} placeholder="Replace with" className="h-8 w-48" />
            <Button variant="outline" size="sm" onClick={findFirst}>Find</Button>
      ...
      <div className="flex min-h-0 flex-1 overflow-hidden">
        <aside className="hidden w-60 shrink-0 overflow-y-auto border-r bg-white p-4 dark:bg-slate-900 xl:block">
          <div className="space-y-5">
            <div>
              ...
              <p className="mt-1 text-xs text-muted-foreground">Stored in the canonical document JSON and used by PDF export.</p>
            </div>
            <div className="space-y-2">
              <Label htmlFor="page-size">Page size</Label>
              <select id="page-size" value={content.page.size} onChange={(event) => updatePage({ size: event.target.value === "LETTER" ? "LETTER" : "A4" })} className="h-9 w-full rounded-md border bg-background px-3 text-sm">
            ...
            <div className="space-y-2">
              ...
              <select id="orientation" value={content.page.orientation} onChange={(event) => updatePage({ orientation: event.target.value === "landscape" ? "landscape" : "portrait" })} className="h-9 w-full rounded-md border bg-background px-3 text-sm">
            ...
            <div className="space-y-2">
              ...
              <select
                ...
                onChange={async (event) => {
                  try {
                    const updated = await letterheadFn({ data: { documentId: document.id, letterheadId: event.target.value || null } });
                    onDocumentUpdated?.(updated);
              ...
              >
                ...
                {(letterheads ?? []).map((letterhead) => <option key={letterhead.id} value={letterhead.id}>{letterhead.name}</option>)}
              </select>
            </div>
              ...
              {(["top", "right", "bottom", "left"] as const).map((side) => (
                <div key={side} className="space-y-1">
                  ...
                  <Input
                    ...
                    onChange={(event) => updatePage({ margins: { ...content.page.margins, [side]: Number(event.target.value) || 20 } })}
                    className="h-8"
                  />
                </div>
              ))}
            ...
            <div className="space-y-2">
              <Label>Header</Label>
              <Textarea rows={2} value={content.page.header} onChange={(event) => updatePage({ header: event.target.value })} placeholder="Optional document header" />
            </div>
            <div className="space-y-2">
              <Label>Footer</Label>
              <Textarea rows={2} value={content.page.footer} onChange={(event) => updatePage({ footer: event.target.value })} placeholder="Optional document footer" />
            </div>
            <label className="flex items-center gap-2 text-sm">
              <input type="checkbox" checked={content.page.showPageNumbers} onChange={(event) => updatePage({ showPageNumbers: event.target.checked })} />
              Show page numbers
            </label>
          </div>
        </aside>
        ...
        <div className="min-w-0 flex-1 overflow-auto p-4 sm:p-8">
          <div className="mb-3 flex items-center justify-center gap-2 text-xs text-muted-foreground">
            <Button variant="outline" size="sm" className="h-7 px-2" onClick={() => setZoom((value) => Math.max(75, value - 25))}>−</Button>
            <span className="w-12 text-center">{zoom}%</span>
            <Button variant="outline" size="sm" className="h-7 px-2" onClick={() => setZoom((value) => Math.min(150, value + 25))}>+</Button>
          </div>
          <div className="mx-auto origin-top transition-transform" style={{ width: orientedWidth, transform: `scale(${zoom / 100})` }}>
            <div
            ...
            >
              ...
              <div
                ...
                style={{ paddingTop: marginPx.top, paddingRight: marginPx.right, paddingBottom: marginPx.bottom, paddingLeft: marginPx.left }}
              />
              {(content.page.footer || content.page.showPageNumbers) && (
                <div className="absolute bottom-0 left-0 right-0 flex items-center justify-between border-t px-8 py-3 text-[11px] text-slate-500">
                  ...
                  {content.page.showPageNumbers && <span>PDF page numbers generated on export</span>}
                </div>
              )}
            </div>
          </div>
      ...
      {saveState === "conflict" && (
        <div className="border-t border-red-200 bg-red-50 px-4 py-3 text-sm text-red-800 dark:border-red-900 dark:bg-red-950/30 dark:text-red-200">
          Editing is paused because another session saved a newer version. Reload this document before making more changes.
        </div>
      )}
      ...
      <Dialog open={versionDialogOpen} onOpenChange={setVersionDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Create an immutable structured snapshot before a major change or review.</DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-2">
            <div className="space-y-2"><Label>Version title</Label><Input value={versionTitle} onChange={(event) => setVersionTitle(event.target.value)} placeholder="e.g. Review draft" /></div>
            <div className="space-y-2"><Label>Change summary</Label><Textarea rows={3} value={versionSummary} onChange={(event) => setVersionSummary(event.target.value)} placeholder="What changed in this version?" /></div>
          </div>
          <DialogFooter><Button variant="outline" onClick={() => setVersionDialogOpen(false)}>Cancel</Button><Button onClick={() => void createVersion()} disabled={saveState === "saving"}>{saveState === "saving" && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}Save snapshot</Button></DialogFooter>
        </DialogContent>
      </Dialog>

      <Dialog open={historyOpen} onOpenChange={setHistoryOpen}>
        <DialogContent className="max-h-[80vh] overflow-hidden sm:max-w-2xl">
          <DialogHeader><DialogTitle>Version history</DialogTitle><DialogDescription>Immutable native-document snapshots. Restoring creates an automatic backup of the current content first.</DialogDescription></DialogHeader>
          <div className="max-h-[56vh] space-y-2 overflow-y-auto pr-1">
            {(versions ?? []).length === 0 ? (
              <div className="rounded-lg border border-dashed p-8 text-center text-sm text-muted-foreground">No saved versions yet. Use “Save version” to create the first snapshot.</div>
            ) : (
              (versions ?? []).map((version) => (
                <div key={version.id} className="flex items-start justify-between gap-4 rounded-lg border p-3">
                  <div className="min-w-0">
                    <p className="font-medium">v{version.version_number} {version.title ? `— ${version.title}` : ""}</p>
                    <p className="mt-1 text-xs text-muted-foreground">{new Date(version.created_at).toLocaleString()} • {version.word_count ?? 0} words</p>
                    {version.change_summary && <p className="mt-2 text-sm text-muted-foreground">{version.change_summary}</p>}
                  </div>
                  <Button variant="outline" size="sm" disabled={restoringVersionId === version.id} onClick={() => void restoreVersion(version.id)}>{restoringVersionId === version.id && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}Restore</Button>
                </div>
              ))
            )}
          </div>
      ...
      <Dialog open={linkOpen} onOpenChange={setLinkOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader><DialogTitle>Add link</DialogTitle><DialogDescription>Apply a web or email link to the current text selection.</DialogDescription></DialogHeader>
          <div className="space-y-2 py-2"><Label>URL</Label><Input value={linkUrl} onChange={(event) => setLinkUrl(event.target.value)} placeholder="https://example.com" onKeyDown={(event) => { if (event.key === "Enter") applyLink(); }} /></div>
          <DialogFooter><Button variant="outline" onClick={() => setLinkOpen(false)}>Cancel</Button><Button onClick={applyLink}>Apply link</Button></DialogFooter>
        </DialogContent>
      </Dialog>
