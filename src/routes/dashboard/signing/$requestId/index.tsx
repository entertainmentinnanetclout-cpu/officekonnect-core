...
export const Route = createFileRoute("/dashboard/signing/$requestId/")({
  component: SigningRequestWorkspace,
});
...
function SigningRequestWorkspace() {
  ...
  const myParticipant = useMemo(
    () =>
      (participants ?? []).find((participant) => participant.user_id === context?.user.id) ?? null,
  ...
  const eligible = Boolean(
    request && myParticipant && isSigningParticipantEligible(request, myParticipant),
  );
  ...
  const declineMutation = useMutation({
    mutationFn: () =>
      declineFn({ data: { participantId: myParticipant!.id, reason: declineReason } }),
    onSuccess: async () => {
      setDeclineOpen(false);
      toast.success("Signing request declined");
      await queryClient.invalidateQueries({ queryKey: ["signing-request", requestId] });
      await queryClient.invalidateQueries({ queryKey: ["signing-participants", requestId] });
    },
  ...
  const finalizeMutation = useMutation({
    ...
    onSuccess: async () => {
      toast.success("Finalization completed");
      await queryClient.invalidateQueries({ queryKey: ["signing-request", requestId] });
      await queryClient.invalidateQueries({ queryKey: ["signing-assets", requestId] });
    },
  ...
  const rotateInvite = async (participant: Participant) => {
    try {
      const result = await rotateFn({
        data: { participantId: participant.id, expiresAt: request?.expires_at },
      });
      ...
      toastError(error, "Could not rotate invitation");
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span
              className={`rounded-full border px-2.5 py-1 text-xs font-medium ${requestTone(request.status)}`}
            >
              {signingStatusLabel(request.status)}
            </span>
          </div>
          <p className="mt-1 text-sm text-muted-foreground">
            {request.message || "OfficeKonnect secure e-signature request"}
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          {request.status === "draft" && isSender && (
            <Button asChild>
          ...
          {isSender && ["sent", "in_progress"].includes(request.status) && (
            <Button variant="outline" onClick={() => setCancelOpen(true)}>
              Cancel request
            </Button>
          )}
          {isSender && ["queued", "failed"].includes(request.finalization_status) && (
          ...
          {assets?.finalUrl && (
            <Button variant="outline" asChild>
              <a href={assets.finalUrl} target="_blank" rel="noreferrer">
          ...
          {assets?.certificateUrl && (
            <Button variant="outline" asChild>
              <a href={assets.certificateUrl} target="_blank" rel="noreferrer">
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {[
          ...
          ["Current turn", String(request.current_order_index + 1)],
          ["Finalization", request.finalization_status],
          [
            "Expires",
            request.expires_at ? format(new Date(request.expires_at), "PPp") : "No expiry",
          ],
        ].map(([label, value]) => (
          <Card key={label}>
            <CardContent className="p-4">
              <p className="text-xs text-muted-foreground">{label}</p>
              <p className="mt-1 truncate font-semibold">{signingStatusLabel(value)}</p>
            </CardContent>
          </Card>
        ))}
      </div>
      ...
      {myParticipant && (
        <Card className={eligible ? "border-violet-300" : undefined}>
          <CardHeader>
            ...
            {eligible ? (
              <>
                <p className="text-sm text-muted-foreground">
                  Complete the fields assigned to you. The server verifies identity, signing order,
                  consent and the locked participant/field hashes before accepting completion.
                </p>
                {myFields.length > 0 && assets?.sourceUrl && (
                  <div className="overflow-auto rounded-lg border bg-slate-100 p-3 dark:bg-slate-900">
                    ...
                    <div key={field.id} className="rounded-lg border p-3">
                      ...
                      {field.type === "signature" || field.type === "initial" ? (
                        ...
                        <Input
                          className="mt-3"
                          type="date"
                          value={fieldValues[field.id] ?? format(new Date(), "yyyy-MM-dd")}
                          onChange={(event) =>
                ...
                <label className="flex items-start gap-3 rounded-lg border bg-muted/20 p-3 text-sm">
                  ...
                  />
                  <span>
                    I consent to use an electronic signature for this request and understand that
                    OfficeKonnect records signing consent, timestamps and integrity hashes in the
                    audit trail.
                  </span>
                </label>
                <div className="flex flex-wrap justify-end gap-2">
                  ...
                  <Button
                    ...
                    disabled={!consent || completeMutation.isPending}
                  >
              ...
              <div className="rounded-lg bg-muted p-4 text-sm text-muted-foreground">
                {myParticipant.status === "signed"
                  ? "You completed this signing request."
                  : myParticipant.status === "declined"
                    ? "You declined this request."
                    : request.signing_order === "sequential"
                      ? "Your signing turn is not active yet. OfficeKonnect will notify you when the preceding participant completes."
                      : "This request is not currently eligible for action."}
              </div>
            )}
          </CardContent>
        </Card>
              ...
              >
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {participant.role} · Turn {participant.order_index + 1}
                  </p>
                </div>
                <span className="rounded-full bg-muted px-2 py-1 text-xs">
                  {signingStatusLabel(participant.status)}
                </span>
                {isSender &&
                  !participant.user_id &&
                  ["sent", "in_progress"].includes(request.status) && (
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => void rotateInvite(participant)}
                    >
              ...
              (events ?? []).map((event) => (
                <div key={event.id} className="flex gap-3 border-b pb-3 last:border-0">
                  ...
                  <div className="min-w-0">
                    <p className="text-sm font-medium">
                      {signingStatusLabel(event.event_type.replaceAll(".", " "))}
                    </p>
                    <p className="text-xs text-muted-foreground">
      ...
      <Dialog open={signatureOpen} onOpenChange={setSignatureOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Use a saved signature, draw one, or create a typed signature. New signatures are
              stored under your workspace identity and referenced by ID during authenticated
              completion.
            </DialogDescription>
          </DialogHeader>
          <Tabs
          ...
          <DialogFooter>
            ...
            >
              {signatureMutation.isPending && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}Use
              signature
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog open={declineOpen} onOpenChange={setDeclineOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Declining is a terminal action for this request and will be recorded in the audit
              trail.
            </DialogDescription>
          </DialogHeader>
          <Textarea
      ...
      <Dialog open={cancelOpen} onOpenChange={setCancelOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Cancellation revokes active participant access and remains visible in the audit trail.
            </DialogDescription>
          </DialogHeader>
          <Textarea
            value={cancelReason}
