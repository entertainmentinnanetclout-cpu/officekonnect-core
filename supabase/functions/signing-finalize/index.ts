...
async function buildCertificate(
  ...
  for (const participant of payload.participants ?? []) {
    ...
    for (const line of wrapText(
      `${identity} — ${participant.role} — ${participant.status} — completed ${participant.signedAt ?? "not applicable"}`,
      82,
    )) {
      drawLine(line, 8);
    }
  ...
  for (const event of payload.events ?? []) {
    for (const line of wrapText(
      `${event.createdAt} — ${event.eventType} — ${event.eventHash ?? "pending hash"}`,
      82,
    )) {
      drawLine(line, 7.5);
    }
...
Deno.serve(async (req: Request) => {
  ...
  if (!supabaseUrl || !anonKey || !serviceKey)
    return json({ error: "Signing finalizer is not configured" }, 500);
  ...
  let claimed = false;
  try {
    const { data: payload, error: claimError } = await service.rpc("claim_signing_finalization", {
    ...
    const certUpload = await service.storage
      .from("exports")
