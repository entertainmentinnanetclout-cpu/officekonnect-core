...
Preserve and extend the existing Supabase data model and state machines. Do not create parallel workflow, signing, document-version, role or storage systems unless a demonstrated gap requires an additive model.
...
Do not create a second native-document table.
