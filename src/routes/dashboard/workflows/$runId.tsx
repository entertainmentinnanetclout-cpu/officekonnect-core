...
function WorkflowReview() {
  ...
  const membersById = useMemo(
    () => new Map((memberDirectory ?? []).map((member) => [member.user_id, member])),
    [memberDirectory],
  );
  ...
  const myAssignment =
    (assignments ?? []).find(
      (assignment) =>
        assignment.user_id === authContext?.userId &&
        assignment.step_id === activeStep?.id &&
  ...
  const canResubmit = Boolean(
    ...
    document &&
    (isAdmin ||
      run.started_by === authContext.userId ||
      document.created_by === authContext.userId),
  );
  const canCancel = Boolean(
    run &&
    !isTerminalWorkflowStatus(run.status) &&
    authContext &&
    (isAdmin || run.started_by === authContext.userId),
  );
  ...
  const decisionMutation = useMutation({
    mutationFn: () => {
      if (!myAssignment || !decision)
        throw new Error("No active assignment is awaiting your decision");
      if ((decision === "changes_requested" || decision === "reject") && !decisionComment.trim()) {
        throw new Error("Add a reason for this decision");
      }
      return decisionServerFn({
        data: { assignmentId: myAssignment.id, decision, comment: decisionComment },
      });
    },
    onSuccess: async () => {
  ...
  const resubmitMutation = useMutation({
    mutationFn: () => {
      ...
      return resubmitServerFn({
        data: {
          runId,
          expectedDocumentEditorVersion: document.editor_version,
  ...
  if (runError || !run) {
    return (
      <div className="rounded-xl border border-red-200 bg-red-50 p-6 text-sm text-red-800 dark:border-red-900 dark:bg-red-950/30 dark:text-red-200">
        {runError instanceof Error ? runError.message : "Workflow could not be loaded."}
      </div>
    );
  }

  const workingDocumentRoute =
    ...
    <div className="space-y-5">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            {run.title} · Workflow revision {run.workflow_revision} · submitted{" "}
            {formatDistanceToNow(new Date(run.started_at), { addSuffix: true })}
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          {workingDocumentRoute && (
            <Button variant="outline" asChild>
              <Link {...workingDocumentRoute}>
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              {(steps ?? []).map((step: WorkflowStep) => (
                <div
                  key={step.id}
                  className={`rounded-lg border p-3 ${step.status === "active" ? "border-blue-300 bg-blue-50/50 dark:border-blue-900 dark:bg-blue-950/20" : ""}`}
                >
                  <div className="flex items-start gap-2">
                    ...
                    <span
                      className={`rounded-full px-2 py-0.5 text-[10px] font-medium ${statusClasses(step.status)}`}
                    >
                  ...
                  {step.due_at && (
                    <p className="mt-2 flex items-center gap-1 text-[11px] text-muted-foreground">
                      ...
                      Due {format(new Date(step.due_at), "d MMM, HH:mm")}
                    </p>
                  )}
                      ...
                      return (
                        <div key={assignment.id} className="flex items-center gap-2 text-[11px]">
                          ...
                          <span className="min-w-0 flex-1 truncate">
                            {member?.full_name || member?.email || assignment.user_id.slice(0, 8)}
                          </span>
                            ...
                            !isTerminalWorkflowStatus(run.status) && (
                              <Button
                                ...
                                onClick={() => {
                                  ...
                                  setReassignUserId(assignment.user_id);
                                }}
                              >
          ...
          <Card>
            <CardContent className="space-y-2 p-4 text-xs">
              <div className="flex justify-between gap-3">
                <span className="text-muted-foreground">Overall due</span>
                <span>
                  {run.due_at ? format(new Date(run.due_at), "d MMM yyyy, HH:mm") : "No deadline"}
                </span>
              </div>
              <div className="flex justify-between gap-3">
                <span className="text-muted-foreground">Submitted editor version</span>
                <span>{run.document_editor_version_at_submission}</span>
              </div>
              <div className="flex justify-between gap-3">
              ...
              <div className="flex justify-between gap-3">
                ...
                <span>{run.template_version ?? "—"}</span>
              </div>
            </CardContent>
          </Card>
        </aside>
        ...
        <aside className="space-y-4">
            ...
            <Card className="border-blue-200 dark:border-blue-900">
              ...
              <CardContent className="space-y-2">
                <p className="text-xs text-muted-foreground">
                  You are the active {myAssignment.participant_role} for “{activeStep.name}”. Your
                  decision is recorded against workflow revision {run.workflow_revision}.
                </p>
          ...
          {run.status === "changes_requested" && (
            <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900 dark:border-amber-900 dark:bg-amber-950/20 dark:text-amber-200">
              ...
              <p className="mt-1 text-xs">
                Edit the working document, then resubmit. The current review snapshot remains
                immutable until the resubmission RPC creates the next document version and workflow
                revision.
              </p>
            </div>
          )}
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2">
                <select
                  ...
                  value={commentStepId ?? ""}
                  onChange={(event) => setCommentStepId(event.target.value || null)}
                >
                ...
                {(comments ?? []).length === 0 ? (
                  ...
                  (comments ?? []).map((comment) => {
                    ...
                    return (
                      <div
                        key={comment.id}
                        className={`rounded-lg border p-3 text-xs ${comment.is_resolved ? "opacity-60" : ""}`}
                      >
                        <div className="flex items-start gap-2">
                          <div className="min-w-0 flex-1">
                            <p className="truncate font-medium">
                              {author?.full_name || author?.email || "Workflow participant"}
                            </p>
                        ...
                        <Button
                          ...
                          onClick={async () => {
                            try {
                              await resolveCommentServerFn({
                                data: { commentId: comment.id, resolved: !comment.is_resolved },
                              });
                              await invalidateReview();
                            } catch (error) {
                              toastError(error, "Comment status could not be changed");
                            }
                          }}
                        >
                          {comment.is_resolved ? (
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
                ...
                return (
                  <div key={event.id} className="border-l-2 pl-3 text-xs">
                    <p className="font-medium">
                      {event.event_type.replaceAll("workflow.", "").replaceAll("_", " ")}
                    </p>
              ...
              {(events ?? []).length === 0 && (
                <p className="text-xs text-muted-foreground">No workflow events yet.</p>
              )}
              {(decisions ?? []).length > 0 && (
                <div className="border-t pt-3 text-[10px] text-muted-foreground">
                  {decisions!.length} immutable decision record{decisions!.length === 1 ? "" : "s"}{" "}
                  across workflow revisions.
                </div>
              )}
            </CardContent>
          </Card>
      ...
      <Dialog open={decisionOpen} onOpenChange={setDecisionOpen}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>
              {decision ? workflowDecisionLabel(decision) : "Workflow decision"}
            </DialogTitle>
            <DialogDescription>
              This action is committed by the workflow RPC and cannot be rewritten as a client-side
              status change.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-2">
      ...
      >
        <DialogContent className="max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              Only the author or a workspace administrator may update a workflow comment.
            </DialogDescription>
          </DialogHeader>
          <Textarea
            rows={5}
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={editCommentMutation.isPending || !editCommentBody.trim()}
            >
              Save comment
            </Button>
          </DialogFooter>
      ...
      >
        <DialogContent className="max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              Reassignment is handled by the existing workflow RPC and recorded in the workflow
              event trail.
            </DialogDescription>
          </DialogHeader>
      ...
      <Dialog open={cancelOpen} onOpenChange={setCancelOpen}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              Cancellation is terminal for this workflow run and is retained in the audit history.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-2">
      ...
      <Dialog open={resubmitOpen} onOpenChange={setResubmitOpen}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>Resubmit current working document</DialogTitle>
            <DialogDescription>
              The backend will snapshot the current editor version into a new immutable document
              version, increment the workflow revision and reopen the review sequence.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3">
            <div className="rounded-lg border bg-muted/30 p-3 text-xs">
              <p className="font-medium">
                Working editor version: {document?.editor_version ?? "—"}
              </p>
              <p className="mt-1 text-muted-foreground">
                Current submitted workflow revision: {run.workflow_revision}
              </p>
            </div>
            <div className="space-y-2">
