...
function SheetsIndex() {
  ...
  const createMutation = useMutation({
    mutationFn: (input?: { title?: string; content?: WorkbookContent }) =>
      createFn({ data: input ?? {} }),
  ...
  const statusMutation = useMutation({
    mutationFn: (input: { documentId: string; status: "draft" | "archived" | "deleted" }) =>
      statusFn({ data: input }),
  ...
  const importSpreadsheet = async (file?: globalThis.File | null) => {
    ...
    try {
      ...
      const entries = source.SheetNames.map((name) => ({
        name,
        matrix: XLSX.utils.sheet_to_json<unknown[]>(source.Sheets[name]!, {
          header: 1,
          raw: true,
          defval: null,
        }) as unknown[][],
      ...
      source.SheetNames.forEach((name, sheetIndex) => {
        ...
        for (const [address, xlsxCell] of Object.entries(sourceSheet)) {
          ...
          const formula =
            "f" in xlsxCell && typeof xlsxCell.f === "string" ? xlsxCell.f : undefined;
          if (!formula) continue;
          const current = targetSheet.cells[address] ?? {};
          targetSheet.cells[address] = {
            ...current,
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Create workbooks, calculate formulas, import Excel/CSV files and prepare print-ready
            PDFs from one secure workspace.
          </p>
        </div>
        ...
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center">
          <div className="relative min-w-0 flex-1">
            ...
            <Input
              ...
              placeholder="Search spreadsheet titles…"
              className="pl-9"
            />
          </div>
          <div className="flex flex-wrap gap-2">
          ...
          <h2 className="mt-4 font-semibold">
            {scope === "active"
              ...
              : scope === "archived"
                ...
                : "Spreadsheet Trash is empty"}
          </h2>
          <p className="mt-1 max-w-md text-sm text-muted-foreground">
            {scope === "active"
              ? "Start a blank OfficeKonnect workbook or import an existing XLSX, XLS or CSV file. No sample production data is created."
              : "Spreadsheet lifecycle actions remain recoverable through the shared document model."}
          </p>
          {scope === "active" && (
            <div className="mt-5 flex gap-2">
              ...
              <Button onClick={() => createMutation.mutate(undefined)}>
            ...
            >
              <CardContent className="p-0">
                <Link
                  ...
                  className="flex h-28 items-center justify-center bg-emerald-50/70 dark:bg-emerald-950/20"
                >
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    >
                      ...
                      <p className="mt-1 text-xs text-muted-foreground">
                        {document.sheet_count.toLocaleString()} sheets ·{" "}
                        {document.cell_count.toLocaleString()} cells ·{" "}
                        {document.formula_count.toLocaleString()} formulas
                      </p>
